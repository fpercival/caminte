...
MongoDB.prototype.create = function (model, data, callback) {
    ...
    this.collection(model).insert(data, {}, function (err, m) {
        ...
        if(inserted && inserted._id) inserted.id = inserted._id.toString();
...
MongoDB.prototype.save = function (model, data, callback) {
    var id = data.id;
//    id = getObjectId(id);
    this.collection(model).update({_id: getObjectId(id)}, data, function (err) {
        callback(err);
    });
};
/**
...
//        var id = getObjectId(filter.id);
        filter._id = getObjectId(filter.id);
        delete filter.id;
    }
...
//    id = getObjectId(id);
    this.collection(model).findOne({_id: getObjectId(id)}, function (err, data) {
...
//    id = getObjectId(id);
    this.collection(model).remove({_id: getObjectId(id)}, callback);
};

MongoDB.prototype.remove = function remove(model, filter, callback) {
    var cond = buildWhere(filter.where, this, model);
...
MongoDB.prototype.all = function all(model, filter, callback) {
    ...
    var query = {};
    if (filter.where) {
        query = buildWhere(filter.where, this, model);
    }
//console.log('mongo q: %j', model, query);
...
MongoDB.prototype.count = function count(model, callback, filter) {
    var cond;
    if(filter.where) cond = buildWhere(filter.where, this, model);
    else cond = {};
...
//    id = getObjectId(id);
    this.collection(model).findAndModify({_id: getObjectId(id)}, [['_id', 'asc']], {$set: data}, {}, callback);
};
...
function buildWhere(filter, adapter, model) {
    var query = {};
...
//                spec = spec === 'like' ? 'regex' : spec;
                if(spec === 'like'){
                    query[k]['$regex'] = cond.replace(/[\%\s]+/gi, '.*');
                    query[k]['$options'] = "i";
                }
                else if (spec === 'nlike') {
                    query[k]['$not'] = new RegExp(cond, 'i');
                } else {
                    if(adapter._models[model].properties[spec]==undefined ) spec='$' + spec;
                    query[k][ spec ] = cond;
                }
            }
        } else {
